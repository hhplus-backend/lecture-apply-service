...
class LectureApplicationServiceTest(
    @Autowired val clock: Clock,
        ...
        given("유효한 강의 ID와 사용자 정보가 주어지고, lock 획득에 성공하면") {
            every { lectureRepository.getLock(lockKey) } returns 1
            every { lectureRepository.getLectureById(lectureId) } returns lecture
            every { lectureRepository.releaseLock(lockKey) } returns 1
        ...
        given("유효한 강의 ID와 사용자 정보가 주어지고, lock 획득에 실패하면") {
            every { lectureRepository.getLock(lockKey) } returns 0
            `when`("applyForLecture 호출될때") {
                val result = lectureApplicationService.applyForLecture(lectureId, user)
